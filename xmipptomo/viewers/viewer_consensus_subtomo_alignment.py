...
from xmipptomo.protocols.protocol_consensus_subtomo_alignment import ScoreTransformOutputs
from pyworkflow.viewer import Viewer

from xmipptomo.protocols import XmippProtConsensusSubtomoAlignnment
from pwem.viewers.plotter import EmPlotter, plt


class XmippConsensusSubtomoAlignment(Viewer):
    """ Visualize the output of protocol reconstruct swarm """
    _label = 'Score transformation viewer'
    _targets = [XmippProtConsensusSubtomoAlignnment]

    def _visualize(self, scoreProtocol: XmippProtConsensusSubtomoAlignnment, **kwargs):
    ...
    def plotAngularDistibution(self, subtomos:SetOfSubTomograms, **kwargs):
        ...
        plotter.plotAngularDistributionFromSet(subtomos, "Angular distribution", weightAttr=XmippProtConsensusSubtomoAlignnment.SCORE_ATTR)
    ...
    def plotScoreHistogram(self, subtomos:SetOfSubTomograms, **kwargs):
        ...
        results = subtomos.getUniqueValues(["id", XmippProtConsensusSubtomoAlignnment.SCORE_ATTR])

        scores = results[XmippProtConsensusSubtomoAlignnment.SCORE_ATTR]
